...
export default function Board() {
	...
	const onDrop = (info: DropProps) => {
		setCards((prevCards) => {
			const payload = prevCards.find((card) => card.id === info.dragId)
			if (!payload) return prevCards
			const cardsCopy = [...prevCards].filter((card) => card.id !== info.dragId)

			// update the rank here to whatever
			const above = prevCards.filter((card) => card.column === info.to.droppableId && card.id !== info.dragId)[info.to.index - 1]
			const below = prevCards.filter((card) => card.column === info.to.droppableId && card.id !== info.dragId)[info.to.index]

			if (above && below) {
				payload.rank = below.rank.between(above.rank)
			} else if (above) {
				payload.rank = above.rank.genNext()
			} else if (below) {
				payload.rank = below.rank.genPrev()
			}
			payload.column = info.to.droppableId

			cardsCopy.push(payload)
			return cardsCopy.sort((a, b) => a.rank.compareTo(b.rank))
		})
	}

	const deleteCard = (info: DropProps) => {
		setCards((prevCards) => [...prevCards].filter((card) => card.id !== info.dragId).sort((a, b) => a.rank.compareTo(b.rank)))
	}

	const getPlaceholder = (element: HTMLElement, type: string) => {
		return (
			<div className="bg-violet-700 rounded w-full transition-transform" style={{height: element.clientHeight, width: element.clientWidth}} />
		)
	}
	...
	return (
		<div className="h-screen w-screen flex gap-3 p-12">
			<DragDropContext placeholder={getPlaceholder}>
					...
					<Droppable key={column.id} droppableId={column.id} accepts="task" onDrop={onDrop}>
						{(provided, snapshot) => (
							<div
								...
								className="w-56 shrink-0 h-fit p-3 bg-violet-400 rounded flex gap-1 flex-col items-center">
								{cards
				...
				<Droppable key={'burn'} droppableId="burn" accepts="task" onDrop={deleteCard}>
					{(provided, snapshot) => (
						<div
							{...provided.droppableProps}
